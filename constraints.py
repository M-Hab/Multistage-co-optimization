...
def Generator_Capacity(m, d_y, r_y, d_gen_cap, r_gen_cap, strategic_nodes, scen_no):
        '''
...
def Generator_Costs(m, d_y, gen_costs, strategic_nodes, d_gen_cap, gen_c, gen_q, scen_no):
        '''
...
def d_Meet_Demand(d_tranches, m, d_y, d_x, d, beta, f, nodes,gen_nodes
                  , strategic_nodes, buses, arc_id, arc_from, arc_to, scen_no):
        '''
...
def r_Meet_Demand(r_tranches, m, r_y, r_x, r, beta,  nodes
          , strategic_nodes, buses, scen_no ,islands,  I):
        '''
        Meet nodal demands, as well as conservation of flow at each bus.
        '''
        r_cons = {}
...
def Costumer_Bathtub_Constraint(m,d_y , r_y , CBT_RHS , strategic_nodes,scen_no,j):
    for n in strategic_nodes:
        ...
        m.update()
def Generator_Bathtub_Constraint_1(m,d_x , r_x , bt1_rhs, gen_nodes, d_tranches, r_tranches, i) :
    for n in gen_nodes: 
...
def r_improve_Big_M( m, scen_no, nodes,r_tranches, r_z1_tranche, r_z2_tranche,j):
    ...
    m.update()
def d_IP_Constraints(m, strategic_nodes, d_y, d_p, n_d,scen):
        '''
...
def r_IP_Constraints(m, strategic_nodes, r_y, r_p, n_d,islands, buses, beta,I,scen):
        '''
        Integer programming constraints to ensure that marginal
        price and residual demand points are monotonic
        '''
